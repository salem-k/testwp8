     ...
     $scope.updateRDV = function(){
      ...
      $scope.updateContact = function(contact) {
       // var dateRDV = $('#dateX').val();
        ...
        }else if($scope.contact.list =="Nouveau Groupe" && checkIfExist(newNameTmp,$scope.groups)){
            LoadingService.error("Ce groupe existe déjà.<br> Vous pouvez le selectionner dans <br>la liste ou en créer un autre.","ContactEditController");
...
//                         $state.go("app.contactList");
                          ...
                          if($rootScope.fromState=="app.buzcardSend")
                          	LoadingService.confirm("Ce contact a bien été enregistré <br>et votre buzcard lui a été envoyée. ", contactId, "ContactEditController");
                          else if($rootScope.fromState=="app.contactShow")
                          //	LoadingService.confirm("La synchronisation a bien été effectuée.", contactId, "ContactEditController");
...
//                          MenuService.setLocalStorage('ReloadContactList',1);$state.go('app.contactList');
                          ...
                          LoadingService.error("La connexion est insuffisante <br>ou saturée. <br> Veuillez réessayer ultérieurement.","ContactEditController");
                        }
                      },function(status){
                        console.error(status);
                        LoadingService.error("La connexion est insuffisante ou <br> saturée. <br> Veuillez réessayer ultérieurement.","ContactEditController");
                      });}
                      else{
                        LoadingService.dismiss();
//                        $ionicHistory.nextViewOptions({
...
//                      $state.go("app.contactList");
                    	...
                    	// $ionicViewSwitcher.nextDirection("back");
                    	//$state.go('app.contactShow', {id: contactId },{reload : true});
                      LoadingService.confirm("Votre connexion est insuffisante <br>ou saturée. <br>Merci de réessayer ultérieurement <br>en cliquant sur « Synchroniser ».", contactId, "ContactEditController");
                        //only for test
//                        MenuService.setLocalStorage('ReloadContactList',1);$state.go('app.contactList');
//                      $ionicHistory.nextViewOptions({
//                        disableBack: true
...
//                              $state.go("app.contactList");
                           ...
                           //     $state.go('app.contactShow', {id: contactId },{reload : true});
                                        ...
                                        $state.go('app.contactShow', {id: contactId },{reload : true});    //only for test
//                                MenuService.setLocalStorage('ReloadContactList',1);$state.go('app.contactList');
                              }
                            });
